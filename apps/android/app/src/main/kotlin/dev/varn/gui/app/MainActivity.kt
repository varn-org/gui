...
class MainActivity : Activity() {
    ...
    // An activity knows where the application is and the view inside it does not, so the four moments the
    // platform gives are turned into the three states the engine has and handed over here.
    ...
    override fun onPause() {
        host?.report("inactive")
        super.onPause()
    }

    override fun onStop() {
        host?.report("background")
        super.onStop()
    }
    ...
    override fun onTrimMemory(level: Int) {
        ...
        // The levels below this one are no longer delivered, so the one the system still sends when the
        // interface goes out of sight is what asks for what can be worked out again.
    ...
    private fun start(surface: VarnBoxView) {
        try {
            ...
            host.onProblem = { problem -> report(problem) }
            ...
            host.start(
                ...
                File(cacheDir, "varn-gui").absolutePath,
                intent?.data?.toString() ?: "/",
            )
        } catch (problem: Exception) {
            report("the gallery failed to start: ${problem.message}")
        }
    }
     ...
     *
     * An activity that is already open is handed the link here rather than being started again, so the
     * router opens on it instead of the application beginning from nothing.
     */
    override fun onNewIntent(next: Intent) {
        super.onNewIntent(next)
        intent = next

        next.data?.let { host?.open(it.toString()) }
    }
    ...
    private fun unpack(name: String): File {
        ...
        ZipInputStream(assets.open(name)).use { source ->
            ...
            while (entry != null) {
                ...
                // A zip carries the names of the files it holds, and a name may point out of the
                // directory it is expanded into, which is a file written wherever it says.
                check(target.canonicalPath.startsWith(root.canonicalPath + File.separator)) {
                    "the framework carries an entry outside it: ${entry.name}"
                }
